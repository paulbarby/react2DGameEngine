...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const resumeContext = () => { if (audioContext.state === 'suspended')
                audioContext.resume().catch(e => console.error(e)); document.removeEventListener('click', resumeContext); document.removeEventListener('keydown', resumeContext); };
        ...
        yield settingsManager.loadSettings();
        ...
        const demoProject = { startScene: demoScene.name, scenes: { [demoScene.name]: demoScene } };
        // --- Load Assets ---
        try {
            updateStatus('Loading assets...');
            const manifest = yield assetLoader.loadManifest('/assets/manifest.json');
        ...
        if (!currentScene) {
            updateStatus('Error getting current scene.');
            return;
        }
        ...
        eventBus.subscribe('collisionDetected', (event) => {
            ...
            if ((compA.group === 'enemy' && compB.group === 'bullet') || (compA.group === 'bullet' && compB.group === 'enemy')) {
                ...
                if (objectManager.getObjectById(enemyObj.id))
                    objectManager.destroyObject(enemyObj.id);
                updateStatus('Player hit!');
            }
        });
        eventBus.subscribe('gameObjectDestroyed', (event) => {
            const destroyedEvent = event;
        ...
        const originalLoop = gameLoop.loop;
        gameLoop.loop = (currentTime) => {
